...
async function runCommand(file, args, timeout = 20_000) {
...
function statusScript(detailed) {
    return `
...
function parseSpaces(output) {
    const markerIndex = output.lastIndexOf(STATUS_MARKER);
    if (markerIndex < 0)
        throw new Error(output || 'Ego Browser did not return status data');
...
async function getStatus(detailed = false) {
    ...
    if (!binary)
        return { supported: true, installed: false, running: false, spaces: [], pageCount: 0 };
    const version = await installedVersion(binary);
    if (!version)
        return { supported: true, installed: false, running: false, spaces: [], pageCount: 0 };
    const running = await processRunning();
    if (!running)
        return { supported: true, installed: true, running: false, version, spaces: [], pageCount: 0 };
    try {
...
async function launchEgo() {
    if (process.platform !== 'darwin')
        throw new Error('Ego Lite currently supports macOS only.');
...
function unkey(value) {
    return JSON.parse(Buffer.from(value, 'base64url').toString('utf8'));
}
function compactTitle(title, fallback, maxLength = 32) {
...
function ownershipText(ownership, t) {
    if (ownership === 'agent')
        return t('ownership.agent');
    if (ownership === 'agentDelegatedToUser')
        return t('ownership.delegated');
    return t('ownership.user');
}
function statusText(status, t) {
    if (!status.supported)
        return t('status.unsupported', { platform: process.platform });
    if (!status.installed)
        return t('status.notInstalled', { website: WEBSITE });
    if (!status.running)
        return t('status.notRunning', { version: status.version ?? 'unknown' });
    if (status.error)
        return t('status.error', { error: status.error });
...
export function activate(ctx) {
    const t = (key, values) => ctx.i18n.t(key, values);
    ...
    const action = ctx.composerActions.register('ego-browser', {
        async getBadge() {
            const online = cachedStatus?.installed === true
                ...
                && !cachedStatus.error;
            if (cachedStatus?.supported === false)
                return { text: t('badge.unsupported'), active: false };
            return online
        ...
        async execute(_actionContext, itemId, actions) {
            try {
                ...
                if (itemId.startsWith('resume:')) {
                    const spaceId = unkey(itemId.slice('resume:'.length));
                    await actions.composer.fill(t('composer.resumePrompt', { id: spaceId }));
                    return;
                }
                if (itemId.startsWith('close:')) {
                    const spaceId = unkey(itemId.slice('close:'.length));
                    ...
                    if (!result.confirmed)
                        return;
                    const binary = egoBinary();
                    if (!binary || !(await installedVersion(binary)))
                        throw new Error(t('error.notInstalled'));
                ...
                if (itemId.startsWith('tab:')) {
                    const [spaceId, targetId] = unkey(itemId.slice('tab:'.length));
                    await launchEgo();
                    const binary = egoBinary();
