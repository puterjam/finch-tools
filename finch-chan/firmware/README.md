...
pat              # 测试：直接触发"被拍头"的反应（不用真的上手）
stroke           # 测试：直接触发"被撸"的反应（不用真的上手）
...
| **顶部电容区** | `M5StackChan.TouchSensor`（Si12T，三个区 + 前后滑动） | 没屏幕时也能作答：Front = 选项一，Back = 选项二，Middle = 去 Finch；**没有卡片时用来识别"撸"** |
...
| 手势 | 识别方式 | 反应 |
|---|---|---|
| **拍一下** | IMU 尖峰 | 笑脸或爱心（随机）+ 慢慢摇头，3 秒（`kPatReactionMs`） |
| **撸一把** | 0.9 秒内**先后**跨过两个区（`kStrokeMovesNeeded`）：0→1→2 顺毛、0→1→0 来回摸都算 | 笑眼（`Delighted`）+ 小幅摆动，4 秒（`kStrokeReactionMs`） |

撸的反应由 `reactToStroke(overridePat)` 负责：默认和拍头共用防抖窗口，但**允许盖掉刚触发
不久的那次拍头** —— 手刚碰上去时 IMU 通常会先判成"拍了一下"，随后才滑出撸的手势，
这时应该以撸为准（`overridePat=true`）。

两种手势的判定顺序与限制：

- **只有当前没有等待卡片时**才算撸：有卡片时前后滑动是「确认 / 取消」（见 `handleZonePress`），
  这个语义不能被抢走；
- **Si12T 的前后滑动事件在这台硬件上实测不触发**（`wasSwipedForward/Backward` 一直是假），
  所以主力判定是"跨区次数"：同一轮循环里一起报上来的多个区（整只手压上来）不算移动，
  只有**不同时刻**按到**不同的区**才计数；
- 撸的检测窗口 900ms（`kStrokeWindowMs`），超出窗口或手势结束（触发过一次）后重新计数。

不用真的上手也能验证：串口敲 `stroke` / `pat` 直接触发对应反应（见下面的串口命令）。
...
- 小程序下发的功能设置走同一个命令通道：`{"type":"command","action":"settings","music":?,"gain":?,"beat":?}`
  ...
  - **收音灵敏度**：`gain` 0/1/2 → 麦克风线性增益 **1.8 / 2.4 / 4.5**（`kGainLevelGains`，原先是编译期常量）。
    这是进 FFT 前的乘数、没有硬上限，想更灵敏直接改这个表；调太大时正常音量会把频谱顶满（动态范围变小）。
